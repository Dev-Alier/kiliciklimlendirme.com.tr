                            ...
                            @foreach ($list as $key => $lst)
                                <tr data-id="1">
                                    <td data-field="image" style="width: 80px">
                                        @foreach (App\Models\Image::where('product_id',$lst->id)->get()->take(1) as $image)
                                    ...
                                    <td class="process">
                                        <a class="btn btn-outline-secondary btn-sm edit"
                                            ...
                                            href="{{ route('edit.product', $lst->id) }}" type="submit" title="Edit">
                                            <i class="fas fa-pencil-alt"></i> Düzenle
                                        </a>
                                        <a>
                                            <form method="post" action="{{ route('delete.product', ['id' => $lst->id]) }}">
                                                @method('delete')
                                                @csrf
